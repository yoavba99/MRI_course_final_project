...
def parse_args():
    ...
    parser.add_argument("--test_csv", type=str, default="/gpfs0/tamyr/projects/data/brain_age/metadata_age_prediction_test.csv")
...
def save_qualitative_image(image_tensor, true_age, baseline_pred, vit_pred, case_name, out_path):
    ...
    # Assuming shape is (D, H, W). Take middle slice in depth
...
def main():
    ...
    datamodule = MRIDataModule(
        ...
        batch_size=8,  # Small batch size to fit in memory easily
        num_workers=4
    )
    datamodule.setup()
    ...
    if not args.ckpt or not os.path.exists(args.ckpt):
        print(f"Error: Provided checkpoint path '{args.ckpt}' does not exist.")
        print("Please train the model using 'python src/train.py' first and pass the generated checkpoint path via --ckpt.")
        return
    ...
    # We will save images for qualitative analysis
    all_images = []
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(test_loader, desc="ViT Inference"):
            ...
            vit_true.extend(labels.cpu().squeeze().numpy().tolist())
            vit_pred.extend(est_age.cpu().squeeze().numpy().tolist())
            
            # Store images for qualitative extraction (store cpu copies)
    ...
    def get_best_idx(indices, preference="min_sum"):
        ...
        if preference == "min_sum":
            ...
            return indices[np.argmin(base_errors[indices] - vit_errors[indices])] # Minimize base err, maximize vit err
        elif preference == "vit_good":
            return indices[np.argmin(vit_errors[indices] - base_errors[indices])] # Minimize vit err, maximize base err
    
    cases = {
        "Both_Models_Performed_Well": get_best_idx(case1_idx, "min_sum"),
        "Both_Models_Performed_Poorly": get_best_idx(case2_idx, "max_sum"),
        "Baseline_Well_ViT_Poorly": get_best_idx(case3_idx, "base_good"),
        "ViT_Well_Baseline_Poorly": get_best_idx(case4_idx, "vit_good")
    }
