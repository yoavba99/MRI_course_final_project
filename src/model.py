...
# Age bins for soft classification
BIN_WIDTH = 1.0
...
def inject_drop_path(model, drop_path_rate=0.1):
    """Inject DropPath into MONAI TransformerBlocks."""
...
def load_pretrained_vit_weights(model: nn.Module, pretrained_path: str):
    """Load pre-trained ViT weights, skipping mismatched shapes."""
...
class BaseBrainAgeModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = AdamW(self.parameters(), lr=self.hparams.learning_rate, weight_decay=0.05)
...
class SimpleViT3D(BaseBrainAgeModel):
    def __init__(self, img_size=(90, 120, 99), patch_size=(16, 16, 16), stride=None,
